...
class OltService
{
    ...
    public function findByUsername(Router $router, string $username, ?string $callerId = null): ?array
    {
        ...
        try {
            ...
            if (count($jar->toArray()) === 0) {
                Log::warning('OLT PHP session tidak terbentuk, memakai fallback Python.', [
                    'username' => $username,
                    'base_url' => $base,
                ]);
                ...
                if ($pythonResult) {
                    Log::info('OLT ONU cocok melalui Python fallback.', [
    ...
    protected function lookupWithPython(array $oltConfig, string $username, ?string $callerId): ?array
    {
        ...
        $python = trim((string) env('OLT_PYTHON', ''), " \"'");

        // Di Windows, jangan escapeshellarg() nama executable karena hasilnya
        // dapat ditolak oleh cmd.exe. Gunakan launcher Python Windows.
        if ($python === '') {
            $python = PHP_OS_FAMILY === 'Windows' ? 'py -3' : 'python3';
        }
