@switch($setting->type)
    ...
    @case('boolean')
        <select
            name="settings[{{ $setting->id }}]"
            class="form-control">
            <option value="true" {{ $setting->value == 'true' ? 'selected' : '' }}>Aktif</option>
            <option value="false" {{ $setting->value == 'false' ? 'selected' : '' }}>Non Aktif</option>
        </select>
        @break
    ...
    @case('string')
        @if($setting->key == 'whatsapp.provider')
            <select
                name="settings[{{ $setting->id }}]"
                class="form-control">
                <option value="fonnte" {{ $setting->value == 'fonnte' ? 'selected' : '' }}>
                    Fonnte
                </option>
            </select>
            <small class="text-muted">
                Provider yang tersedia dan aktif saat ini: Fonnte.
            </small>
        @else
            <input
                type="text"
